...
colnames(df) <- c("Calling_No", "LRN", "Called_No", "Date", "Time", "Duration", "Cell_1", "Cell_2", "Communication_Type",
                  "IMEI", "IMSI", "Type", "SMSC", "Roam", "G", "isRoam")

drops <- c("LRN","SMSC","G","isRoam","Type") # These are the columns you want that may be removed, change them as per your will
df = df[ , !(names(df) %in% drops)] # drop the columns from the dataframe

# Make hyphens empty in Cell_1 and Cell_2 columns
df$Cell_1[df$Cell_1 == "N/A"] <- ""
df$Cell_2[df$Cell_2 == "N/A"] <- ""
df$Roam[df$Roam == "N/A"] <- ""

# Trim 12 digit mobile numbers to 10 digits
df$Calling_No[nchar(df$Calling_No) == 12] = substr(df$Calling_No[nchar(df$Calling_No) == 12],3,100)
df$Called_No[nchar(df$Called_No) == 12] = substr(df$Called_No[nchar(df$Called_No) == 12],3,100)
df$IMEI = substr(df$IMEI,2,100)

df$Time <- times(df$Time) # Change time from default decimal format to proper HH::MM::SS format

# Microsoft Excel date gives some offset, remove it using this and change format to proper DD/MM/YY format
df$Date <- chron(df$Date, format = c(dates = "d-m-y"), out.format= c(dates = "d/m/y"))
...
INTERNET_List = apply(X=U, MARGIN=1, FUN=function(x1) f(x = x1, t="GPD"))
# Total communication count between all B-parties
TOTAL_List = SMS_IN_List+SMS_OUT_List+CALL_IN_List+CALL_OUT_List+INTERNET_List

# Create pivot dataframe: B-Party and communication count details
df_pivot <- data.frame(U,CALL_IN_List,CALL_OUT_List,SMS_IN_List,SMS_OUT_List,INTERNET_List,TOTAL_List)
colnames(df_pivot) <- c("B_Party", "IN", "OUT", "SMS_IN", "SMS_OUT", "Internet", "Total") # give column names
